...
function buscarPokemon(deslocamento) {
    fetch(`http://pokeapi.co/api/v2/pokemon?offset=${deslocamento}&limite=${limite}`).then((resposta) => resposta.json()).then((dados) => { exibirPokemon(dados.results); alternarBotoes(deslocamento > 0, dados.next != null); });
}
